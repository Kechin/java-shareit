...
//
//        Map<Item,List<Comment>> comments = commentRepository.findAllByItemIn(items).stream().collect(Collectors.groupingBy(Comment::getItem));
...
////
////
////           List <CommentDto> itemComments=CommentMapper.toCommentDtos(comments.get(item));
////            System.out.println("itemComments"+itemComments);
////            setLastAndNextBookings(itemDtoWithBooking);
////            itemDtoWithBooking.setComments(Collections.emptyList());
////            if(itemComments!=null){
////                System.out.println(comments.get(item));
////       itemDtoWithBooking.setComments(itemComments);
////            }
////
...
////        }
//  //      return itemDtos;
//        return itemDtos.stream().map(i -> setLastAndNextBookings(i)).map(i -> setComments(i)).collect(Collectors.toList());
//    }
//
//    @Override
    ...
    public List<ItemDtoWithBooking> getAllByUserId(Long userId, Integer from, Integer size) {
        ...
        for (Item item : items) {
            ...
            if (itemBookings != null) {
                next = itemBookings.stream().filter(i -> i.getStart().isAfter(LocalDateTime.now())).findFirst().get();
                last = itemBookings.stream().filter(i -> i.getEnd().isBefore(LocalDateTime.now())).findFirst().get();
            }
